...
- Activation result naming is only clarified at the backend-facing result-name boundary; the internal `TabPagerActivationResult::Activated` enumerator still names request dispatch imprecisely and remains follow-up work under the activation request-versus-confirmation finding.
