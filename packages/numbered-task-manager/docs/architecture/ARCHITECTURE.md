...
- Keep context-menu task-activity checked-state policy in
  ...
  remain in `TaskActivityLogic.mjs`, while `TaskContextMenuActionSectionsLogic.mjs`
  composes the focused helper output into the full menu sections. QML menu items should
...
- Keep context-menu virtual-desktop action descriptors in
  ...
  and checked-state primitives.
- Keep context-menu basic action availability in `TaskContextMenuWindowActionLogic.mjs`.
...
- Keep checkable context-menu window action state in `TaskContextMenuWindowActionLogic.mjs`.
...
- Apply launcher-list writes through root-owned effect ports that always release their update guard. `LauncherSyncAdapter.qml` should consume a narrow launcher sync port for launcher-list reads/writes and configuration reads/writes instead of receiving the full `TasksModel`. Keep write sequencing, post-write convergence checks, bounded next-change reconciliation state, and sync-failure action-result adaptation in `LauncherSyncLogic.mjs` so failed model/config writes can be unit-tested without QML effects, emitted through the shared action-result logger, and retried once with the attempted launcher list.
