...
- Keep task visual metrics in `TaskMetricsLogic.js`. Delegate size, title visibility threshold, slot width cap, natural-width minima, icon extent, and theme-margin adjustment should have one tested owner instead of being duplicated by root layout, normal task, and remote-attention delegates.
...
- Keep shared task-like content row geometry in `TaskLikeContentRow.qml`. Normal and remote-attention delegates should keep their variant child content, while themed-frame content margins, internal Kirigami spacing, and content opacity application live in one row component.
