...
- Keep task visual metrics in `TaskMetricsLogic.js`. Delegate size, title visibility threshold, task-like title visibility, slot width cap, natural-width minima, icon extent, and theme-margin adjustment should have one tested owner instead of being duplicated by root layout, normal task, and remote-attention delegates.
