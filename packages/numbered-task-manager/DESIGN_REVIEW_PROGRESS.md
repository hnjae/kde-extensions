...
- Interaction helpers: normal task drag MIME payload creation, source-index parsing, drop acceptance policy, and task-like context-menu request payload construction now live in `TaskInteractionLogic.js`. Remaining interaction-helper work is extracting the duplicated task-like zero-delay timer and key/right-click trigger pattern shared by normal and attention delegates.
