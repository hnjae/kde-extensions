...
- Context menu: QML-local role snapshot passthrough wrappers have been removed, activity/virtual-desktop entry construction now comes from tested helper snapshots, New Instance/Move/Resize, Minimize/Maximize, Keep Above/Below, Fullscreen/Shade/No Border, virtual desktop state, Exclude From Capture, and Close consume tested live-role snapshots, all task-model command descriptors are helper-owned, and Pin/Unpin launcher command selection is helper-owned. Keep the remaining live-role boundary helpers (`roleIds`, `roleSource`, `taskRoles`, and the tested JS `roleData`/`boolRoleData` primitives) until a larger menu action-model or adapter extraction can preserve that boundary with less QML-local code. Remaining context-menu work is consolidating launcher activity mutation adapters and moving broader section descriptors into action-model outputs rather than item-local role reads.
