...
- Root/controller ownership: launcher sync state, guarded model/config writes, reconciliation, and sync diagnostics now live in `LauncherSyncAdapter.qml`; activation effect dispatch now lives in `TaskActivationAdapter.qml`; root still owns normal task store state, context-menu creation, drag/drop mutation orchestration, and unavoidable Plasma model instantiation.
