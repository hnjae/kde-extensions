...
- Scope policy: model filter settings and local qualifiers now have a named owner. Remaining scope cleanup is lower-priority facade removal: older normal-task and remote-attention qualifier exports still exist as compatibility wrappers while dependent tests and call sites are migrated deliberately.
