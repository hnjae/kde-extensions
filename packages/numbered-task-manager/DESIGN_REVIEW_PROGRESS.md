...
- Scope policy: model filter settings and local qualifiers now have a named owner; task-specific qualifier re-export facades and task-activity generic facades have been removed. Remaining scope cleanup is generic activity facade removal from launcher logic without weakening launcher-specific activity serialization and pin membership behavior.
