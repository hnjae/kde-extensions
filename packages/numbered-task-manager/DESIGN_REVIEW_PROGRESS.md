...
- Interaction helpers: normal task drag MIME payload creation, source-index parsing, and drop acceptance policy now live in `TaskInteractionLogic.js`. Remaining interaction-helper work is extracting the duplicated task-like context-menu timer/request pattern shared by normal and attention delegates.
