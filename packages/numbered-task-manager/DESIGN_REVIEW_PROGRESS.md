...
- Visible item composer / remote attention: normal task rendering and rendered remote-attention activation/metadata now consume composed descriptors, and `RemoteAttentionSource.qml` owns the remote-attention model, state snapshot, source-facing count/target snapshot, and activation adapter. The obsolete per-normal-index visible-item lookup API has been removed. Full remote-attention removability still requires shrinking root's rendered binding and context-menu wiring to one source-owned item surface.
