...
- Context menu: QML-local role snapshot passthrough wrappers have been removed, activity/virtual-desktop entry construction now comes from tested helper snapshots, New Instance/Move/Resize, Minimize/Maximize, Keep Above/Below, Fullscreen/Shade/No Border, virtual desktop state, Exclude From Capture, and Close consume tested live-role snapshots, and task activity, virtual desktop, basic action, minimize/maximize, and keep-above/below commands are helper-owned. Keep the remaining live-role boundary helpers (`roleIds`, `roleSource`, `taskRoles`, and the tested JS `roleData`/`boolRoleData` primitives) until a larger menu action-model or adapter extraction can preserve that boundary with less QML-local code. Remaining context-menu work is consolidating remaining window action descriptors and launcher mutation adapters rather than item-local role reads.
